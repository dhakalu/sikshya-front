...
import StudentAssignmentRoutes from '../Components/courses/assignment/StudentAssignmentRoutes'
import ForumIcon from '@material-ui/icons/Forum'
import Discussions from '../Components/courses/discussion'
...
const UnderDevelopmentWrapper = styled.div`
  padding: 10px;
  .feature-list-item {
    margin-top: 10px;
  }
...
export const UnderDevelopment = ({ title, name, notes, features }) => (
  <UnderDevelopmentWrapper>
    <h1 className='title'>
      {title || `${name} fature is under development` }
    </h1>
    <p className='notes'>
      {notes}
    </p>
    {
      features && (
        <ul className='feature-list'>
          {
            features.map((feature, i) => (<li key={i} className='feature-list-item'>{feature}</li>))
          }
        </ul>
      )
    }
